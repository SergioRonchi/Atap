...
Begin VB.Form StampaSaldiUNEP 
   BorderStyle     =   4  'Fixed ToolWindow
   Caption         =   "Stampa Saldi UNEP"
   ClientHeight    =   3465
   ClientLeft      =   45
   ClientTop       =   210
   ClientWidth     =   5025
   ...
   Begin VB.PictureBox PictureUNEP 
      ...
      Height          =   495
      Left            =   120
      Picture         =   "StampaSaldiUNEP.frx":0000
      ScaleHeight     =   465
      ScaleWidth      =   465
      TabIndex        =   16
      Top             =   2880
   ...
   Begin VB.Frame FrmProvvisoria 
      ...
      Begin TDBDate6Ctl.TDBDate TxtRicDataIn 
         ...
         _ExtentX        =   2990
         _ExtentY        =   450
         Calendar        =   "StampaSaldiUNEP.frx":0442
         Caption         =   "StampaSaldiUNEP.frx":055A
         ...
         EndProperty
         DropDown        =   "StampaSaldiUNEP.frx":05C6
         Keys            =   "StampaSaldiUNEP.frx":05E4
         Spin            =   "StampaSaldiUNEP.frx":0642
         AlignHorizontal =   0
         AlignVertical   =   0
         Appearance      =   1
      ...
      Begin TDBDate6Ctl.TDBDate TxtRicDataFin 
         ...
         _ExtentX        =   2990
         _ExtentY        =   450
         Calendar        =   "StampaSaldiUNEP.frx":066A
         Caption         =   "StampaSaldiUNEP.frx":0782
         BeginProperty Font {0BE35203-8F91-11CE-9DE3-00AA004BB851} 
            Name            =   "MS Sans Serif"
            Size            =   8.25
            Charset         =   0
            ...
            Strikethrough   =   0   'False
         EndProperty
         DropDown        =   "StampaSaldiUNEP.frx":07EE
         Keys            =   "StampaSaldiUNEP.frx":080C
         Spin            =   "StampaSaldiUNEP.frx":086A
         AlignHorizontal =   0
         AlignVertical   =   0
         Appearance      =   1
         BackColor       =   -2147483643
   ...
   End
End
Attribute VB_Name = "StampaSaldiUNEP"
...
Private Sub CmdOK_Click()
    ...
    If Not GetADORecordset("PrtSaldiUNEP", "*", "1=1", g_Settings.DBConnection) Is Nothing Then
            Call Stampa.gestioneReport("PrtSaldi", "", 0, crptToWindow, "SaldiUNEP.rpt", 1)
       Else
...
Dim visitato As Boolean
    ...
    qry1 = ""
    qry2 = ""
    qrySQL = " SELECT SaldiUNEP.codice, AnagraficaAvvocati.NOME, " & _
             "MID(Chiusura,7,2) & '/' & MID(Chiusura,5,2) & '/' & MID(Chiusura,1,4) , " & _
             "SaldiUNEP.Commento,SaldiUNEP.numOrdinamento,'" & IIf(OptTipoStampa(1).value, "N", "P") & "'," & _
             "SaldiUNEP.SaldoAdempEuro,SaldiUNEP.SaldoSfpgEuro,SaldiUNEP.SaldoNotifEuro,SaldiUNEP.SaldoDecrIngEuro," & _
             "SaldiUNEP.SaldoTotaleEuro,'E' "
    qrySQL = qrySQL & " FROM SaldiUNEP INNER JOIN AnagraficaAvvocati ON SaldiUNEP.Codice = AnagraficaAvvocati.CODAVV "

    visitato = False
    
    If Trim(TxtCodiceAvvocato.Text) <> "" Then
        qry1 = "(SaldiUNEP.Codice = '" & TxtCodiceAvvocato.Text & "')"
    End If
    
    If Trim(TxtRicDataIn.Text <> "") And Trim(TxtRicDataFin.Text <> "") Then
        qry2 = "( SaldiUNEP.Chiusura >= '" & Format(TxtRicDataIn.Text, "yyyymmdd") & "')"
        qry2 = qry2 & " AND ( SaldiUNEP.Chiusura <= '" & Format(TxtRicDataFin, "yyyymmdd") & "')"
    Else
        If Trim(TxtRicDataIn.Text <> "") And Trim(TxtRicDataFin.Text = "") Then
            qry2 = "( SaldiUNEP.Chiusura >= '" & Format(TxtRicDataIn.Text, "yyyymmdd") & "')"
        End If
        If Trim(TxtRicDataIn.Text = "") And Trim(TxtRicDataFin.Text <> "") Then
            qry2 = "( SaldiUNEP.Chiusura <= '" & Format(TxtRicDataFin.Text, "yyyymmdd") & "')"
        End If
    End If
    ...
    If visitato = False Then
        If OptTipoStampa(1).value = True Then
                qrySQL = qrySQL & " WHERE  (((SaldiUNEP.SaldoTotaleEuro)<=-" & Str(g_Settings.LimiteSaldo) & "))"
        Else
                qrySQL = qrySQL & " WHERE (((SaldiUNEP.SaldoTotaleEuro)>-" & Str(g_Settings.LimiteSaldo) & " And (SaldiUNEP.SaldoTotaleEuro)<" & Str(g_Settings.LimiteSaldo) & " And (SaldiUNEP.SaldoTotaleEuro)<>0))"
        End If
    Else
        If OptTipoStampa(1).value = True Then
                qrySQL = qrySQL & " AND  (((SaldiUNEP.SaldoTotaleEuro)<=-" & Str(g_Settings.LimiteSaldo) & "))"

        Else
                qrySQL = qrySQL & " AND (((SaldiUNEP.SaldoTotaleEuro)>-" & Str(g_Settings.LimiteSaldo) & " And (SaldiUNEP.SaldoTotaleEuro)<" & Str(g_Settings.LimiteSaldo) & " And (SaldiUNEP.SaldoTotaleEuro)<>0))"
    ...
    qry = "INSERT INTO PRTSaldiUNEP (Codice,NOME,CHIUSURA,COMMENTO,NUMORDINAMENTO," & _
